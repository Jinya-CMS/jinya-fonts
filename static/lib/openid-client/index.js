...
var _CryptoUtils = class _CryptoUtils {
  ...
  static generateUUIDv4() {
    const uuid = UUID_V4_TEMPLATE.replace(/[018]/g, (c) =>
      (+c ^ (_CryptoUtils._randomWord() & (15 >> (+c / 4)))).toString(16),
    );
  ...
  static async customCalculateJwkThumbprint(jwk) {
    ...
    const utf8encodedAndHashed = await _CryptoUtils.hash(
      "SHA-256",
  ...
  static async generateDPoPProof({
    ...
    try {
      ...
      return await JwtUtils.generateSignedJwt(
        header,
        payload,
        keyPair.privateKey,
      );
  ...
  static async generateDPoPJkt(keyPair) {
    try {
      ...
      if (err instanceof TypeError) {
        throw new Error(
          `Could not retrieve dpop keys from storage: ${err.message}`,
        );
      } else {
        throw err;
      }
   ...
   */
  static async generateClientAssertionJwt(
    client_id,
    client_secret,
    audience,
    algorithm = "HS256",
    ...
    if (!hashFunction) {
      throw new Error(
        `Unsupported algorithm: ${algorithm}. Supported algorithms are: HS256, HS384, HS512`,
      );
    }
    const encoder = new TextEncoder();
    ...
    if (features.width == null)
      features.width =
        (_a = [800, 720, 600, 480].find(
          ...
          ? _a
          : 360;
    (_b = features.left) != null
      ? _b
      : (features.left = Math.max(
          0,
          Math.round(window.screenX + (window.outerWidth - features.width) / 2),
        ));
    if (features.height != null)
      (_c = features.top) != null
        ? _c
        : (features.top = Math.max(
            0,
            Math.round(
              window.screenY + (window.outerHeight - features.height) / 2,
            ),
          ));
    return features;
  }
...
var JsonService = class {
  ...
  async getJson(url, { token, credentials, timeoutInSeconds } = {}) {
    ...
    if (
      ...
      logger2.throw(
        new Error(
          `Invalid response Content-Type: ${contentType != null ? contentType : "undefined"}, from URL: ${url}`,
        ),
      );
    }
    if (
      response.ok &&
      this._jwtHandler &&
      (contentType == null ? void 0 : contentType.startsWith("application/jwt"))
    ) {
  ...
  async postForm(
    url,
    { body, basicAuth, timeoutInSeconds, initCredentials, extraHeaders },
  ) {
  ...
  _appendExtraHeaders(headers) {
    ...
    customKeys.forEach((headerName) => {
      if (protectedHeaders.includes(headerName.toLocaleLowerCase())) {
        logger2.warn(
          "Protected header could not be set",
          headerName,
          protectedHeaders,
        );
        return;
      ...
      if (
        preventOverride.includes(headerName.toLocaleLowerCase()) &&
        Object.keys(headers).includes(headerName)
      ) {
        logger2.warn(
          "Header could not be overridden",
          headerName,
          preventOverride,
        );
        return;
      ...
      const content =
        typeof this._extraHeaders[headerName] === "function"
...
var OidcClientSettingsStore = class {
  constructor({
    ...
    this.filterProtocolClaims =
      filterProtocolClaims != null ? filterProtocolClaims : true;
...
var TokenClient = class {
  ...
  async exchangeCode({
    ...
    if (
      ...
      (client_secret === void 0 || client_secret === null)
    ) {
    ...
    switch (this._settings.client_authentication) {
      ...
      case "client_secret_jwt": {
        const clientAssertion = await CryptoUtils.generateClientAssertionJwt(
          client_id,
          client_secret,
          url,
          this._settings.token_endpoint_auth_signing_alg,
        );
        params.append("client_id", client_id);
        params.append(
  ...
  async revoke(args) {
    ...
    logger2.debug(
      `got revocation endpoint, revoking ${(_a = args.token_type_hint) != null ? _a : "default token type"}`,
    );
...
var ResponseValidator = class {
  ...
  async validateSigninResponse(response, state, extraHeaders) {
    ...
    await this._processClaims(
      response,
      state == null ? void 0 : state.skipUserInfo,
      response.isOpenId,
    );
  ...
  async validateRefreshResponse(response, state) {
    ...
    (_a = response.session_state) != null
      ? _a
  ...
  _validateIdTokenAttributes(response, existingToken, nonce) {
    ...
    if (nonce && incoming.nonce !== nonce) {
      logger2.throw(
        new Error("nonce in id_token does not match nonce in client storage"),
      );
    }
    if (existingToken) {
...
var SigninResponse = class {
  ...
  get isOpenId() {
    ...
    return (
      ((_a = this.scope) == null
        ? void 0
        : _a.split(" ").includes(OidcScope)) || !!this.id_token
    );
  }
};
      ...
      if (result[claim] !== values) {
        if (Array.isArray(result[claim]) || Array.isArray(values)) {
          if (this._settings.mergeClaimsStrategy.array == "replace") {
            ...
            const mergedValues = Array.isArray(result[claim])
              ? result[claim]
              : [result[claim]];
...
var OidcClient = class {
  ...
  async processSigninResponse(url, extraHeaders, removeState = true) {
    ...
    try {
      ...
      if (err instanceof ErrorDPoPNonce && this.settings.dpop) {
        const dpopProof = await this.getDpopProof(
          this.settings.dpop.store,
          err.nonce,
        );
  ...
  async useRefreshToken({
    ...
    if (this.settings.refreshTokenAllowedScope === void 0) {
      ...
      const providedScopes =
        ((_a = state.scope) == null ? void 0 : _a.split(" ")) || [];
    ...
    try {
      ...
      if (err instanceof ErrorDPoPNonce && this.settings.dpop) {
        extraHeaders["DPoP"] = await this.getDpopProof(
          this.settings.dpop.store,
          err.nonce,
        );
        result = await this._tokenClient.exchangeRefreshToken({
      ...
      try {
        ...
        if (url) {
          ...
          const intervalInSeconds =
            this._userManager.settings.checkSessionIntervalInSeconds;
          const stopOnError =
            this._userManager.settings.stopCheckSessionOnError;
          const checkSessionIFrame = new CheckSessionIFrame(
            this._callback,
            client_id,
            url,
            intervalInSeconds,
...
var User = class _User {
  ...
  get scopes() {
    var _a, _b;
    return (_b = (_a = this.scope) == null ? void 0 : _a.split(" ")) != null
      ? _b
      : [];
  }
  toStorageString() {
...
var AbstractChildWindow = class {
  ...
  async navigate(params) {
    ...
    const { url, keepOpen } = await new Promise((resolve, reject) => {
      const listener = (e) => {
        ...
        const data = e.data;
        const origin =
          (_a = params.scriptOrigin) != null ? _a : window.location.origin;
        if (
          e.origin !== origin ||
          (data == null ? void 0 : data.source) !== messageSource
        ) {
          return;
        }
        try {
          const state = UrlUtils.readParams(data.url, params.response_mode).get(
            "state",
          );
          if (!state) {
  ...
  static _notifyParent(
    ...
    if (parent) {
      ...
      if (!state) {
        throw new Error(
          "No parent and no state in URL. Can't complete notification.",
        );
      }
  ...
  close() {
    ...
    if (this._frame) {
      if (this._frame.parentNode) {
        ...
        (_a = this._frame.contentWindow) == null
          ? void 0
  ...
  constructor({
    ...
    if (popupSignal) {
      popupSignal.addEventListener("abort", () => {
        var _a;
        void this._abort.raise(
          new Error((_a = popupSignal.reason) != null ? _a : "Popup aborted"),
        );
      });
    }
    if (
    ...
    this._tokenExpiring = async () => {
      ...
      try {
        ...
        if (err instanceof ErrorTimeout) {
          ...
          const maxRetries =
            this._userManager.settings.maxSilentRenewTimeoutRetries;
          const hasReachedLimit =
            maxRetries !== void 0 && this._timeoutRetryCount > maxRetries;
          if (hasReachedLimit) {
            logger2.error(
...
var UserManager = class {
  ...
  async signinRedirect(args = {}) {
    ...
    let dpopJkt;
    if (
      (_a = this.settings.dpop) == null ? void 0 : _a.bind_authorization_code
    ) {
  ...
  async dpopProof(url, user, httpMethod, nonce) {
    var _a, _b;
    const dpopState = await ((_b =
      (_a = this.settings.dpop) == null ? void 0 : _a.store) == null
      ? void 0
